...
Excellent intro to Hooks. Lots of teachers will only show the `React.useState`
and the `React.useEffect` at the beginning of their explanation because in fact
those are the most used hooks. But that's dangerous because the student can
assume those are the only ones that exist and, in consequence, stop learning the
rest of the api.
